...
function themedrive_switcher() {
    ...
    if ( themedrive_is_enabled() ) {
        ...
        echo sprintf(
            ...
            __( 'Live Theme Test is enabled with', 'ch-directs-plugin' ),
            $theme_name
        );
    } else {
        $get_theme  = wp_get_theme();
        ...
        echo sprintf(
            ...
            __( 'Live Theme Test is disabled', 'ch-directs-plugin' ),
            __( 'The active theme is', 'ch-directs-plugin' ),
            $theme_name
        );
    }
...
function themedrive_options_page() {
    ...
    if ( isset( $_POST['button'] ) && 'Enable Theme Drive' == $_POST['button'] ) {
        ...
        $msg_status = sprintf(
            ...
            __( 'Live theme test is enabled with', 'ch-directs-plugin' ),
            $get_theme_name,
            __( 'theme', 'ch-directs-plugin' ),
            esc_url( site_url() ),
            __( 'View Site', 'ch-directs-plugin' )
        );
        ...
        $msg_status = __( 'Live theme test has been disabled.', 'ch-directs-plugin' );
...
?>
<div class="wrap" >
    <h2><?php _e( 'Live Theme Test', 'ch-directs-plugin' ); ?></h2>
    <?php $action_url; ?>
    ...
    <?php wp_nonce_field( 'theme-drive' ); ?>
        <h2><?php _e( 'Instructions', 'ch-directs-plugin' ); ?></h2>
        <ol>
            <li><?php _e( 'Select a theme to preview live on the site from the select box below (lists all installed themes).', 'ch-directs-plugin' ); ?></li>
            <li><?php _e( 'Enable theme test', 'ch-directs-plugin' ); ?></li>
            <li><?php _e( 'Once the theme is ready to go live, disable theme test and activate the theme on the Themes page.', 'ch-directs-plugin' ); ?></li>
        </ol>
   	    <p><?php _e( 'Additionally you may add "?theme=xxx" to a URL, where xxx is the slug of the theme you want to test.', 'ch-directs-plugin' ); ?></p>
        <?php themedrive_switcher(); ?>
        <h2><?php _e( 'User Role', 'ch-directs-plugin' ); ?></h2>
        <p><?php _e( 'Specify the level of users to have access to the selected theme preview.', 'ch-directs-plugin' ); ?></p>
        <p><?php _e( 'By default it is set to 10 (admin only). Level 7 are editors, level 4 are authors and level 1 are contributors.', 'ch-directs-plugin' ); ?></p>
        <p><?php _e( 'The access level is ignored for accessing the site with ?theme=xxx paramaeter.', 'ch-directs-plugin' ); ?></p>
        <p><input style="border-width: inherit; border-style: inherit; border-color: inherit; width: 30px;" name="access_level" id="access_level" value="<?php echo esc_attr( $access_level ); ?>" /> <label for="access_level"><?php _e( 'Access level', 'ch-directs-plugin' ); ?></label></p>
        <p class="submit">
