...
class User_Avatars {
	...
	public function __construct() {
		...
		// Shortcode.
		add_shortcode( 'ch-directs-plugin', [ $this, 'shortcode' ] );
	...
	public function admin_init() {
		...
		add_settings_field(
			...
			__( 'Local Avatar Permissions',	'ch-directs-plugin' ),
			[ $this, 'avatar_settings_field' ],
			'discussion',
			'avatars',
			[ esc_html__( 'Only allow users with file upload capabilities to upload local avatars (Authors and above).', 'ch-directs-plugin' ) ]
		);
	...
	public function edit_user_profile( $profileuser ) {
		...
		<h3><?php _e( 'Avatar', 'ch-directs-plugin' ); ?></h3>
		<table class="form-table">
			<tr>
				<th><label for="basic-user-avatar"><?php _e( 'Upload Avatar', 'ch-directs-plugin' ); ?></label></th>
				...
				if ( empty( $options['chd_user_avatars_caps'] ) || current_user_can( 'upload_files' ) ) {
					...
					if ( empty( $profileuser->chd_user_avatar ) ) {
						...
						echo '<span class="description">' . __( 'Replace the local avatar by uploading a new avatar, or erase the local avatar (falling back to a gravatar) by checking the delete option.', 'ch-directs-plugin' ) . '</span>';
					}
		...
		<form id="basic-user-avatar-form" action="<?php the_permalink(); ?>" method="post" enctype="multipart/form-data">
			...
			if ( empty( $options['chd_user_avatars_caps'] ) || current_user_can( 'upload_files' ) ) {
				...
				if ( empty( $profileuser->chd_user_avatar ) ) {
					...
					echo '<input type="checkbox" name="basic-user-avatar-erase" value="1" /> ' . __( 'Delete local avatar', 'ch-directs-plugin' ) . '<br />';
					echo '<p class="description">' . __( 'Replace the local avatar by uploading a new avatar, or erase the local avatar (falling back to a gravatar) by checking the delete option.', 'ch-directs-plugin' ) . '</p>';
				}
			...
			?>
			<input type="submit" name="manage_avatar_submit" value="<?php _e( 'Update Avatar', 'ch-directs-plugin' ); ?>" />
		</form>
		<?php
	...
	public function bbpress_user_profile() {
		...
		echo '<div>';
			echo '<label for="basic-local-avatar">' . __( 'Avatar', 'ch-directs-plugin' ) . '</label>';
 			echo '<fieldset class="bbp-form avatar">';
				...
				if ( empty( $options['chd_user_avatars_caps'] ) || current_user_can( 'upload_files' ) ) {
					...
					if ( empty( $profileuser->chd_user_avatar ) ) {
						...
						echo '<span class="description" style="margin-left:0;">' . __( 'Replace the local avatar by uploading a new avatar, or erase the local avatar (falling back to a gravatar) by checking the delete option.', 'ch-directs-plugin' ) . '</span>';
					}
