...
class Filter_By_Template {
	...
	public function filter_dropdown() {
		...
		<select name="page_template_filter" id="page_template_filter">
			<option value="all"><?php _e( 'All Page Templates', 'ch-directs-plugin' ); ?></option>
			<option value="default" <?php echo ( $template == 'default' ) ? ' selected="selected" ' : ''; ?>><?php echo _e( 'Default Template', 'ch-directs-plugin' ); ?></option>
	...
	public function template_columns_head( $columns ) {
		...
		$columns['template'] = __( 'Template', 'ch-directs-plugin' );
	        ...
	        if ( $template ) {
	        	...
	        	if ( $template == 'default' ) {
					...
					echo sprintf(
						...
						__( 'Default Template', 'ch-directs-plugin' ),
						__( 'Default Template', 'ch-directs-plugin' )
					);
				...
				// If it's not the default template.
	        		...
	        		if ( isset( $templates[ $template ] ) ) {
	        			echo sprintf(
							...
							__( 'Template file:', 'ch-directs-plugin' ),
							$template,
							$templates[ $template ]
						);
					...
					// If the template cannot be found.
	        			...
	        			echo sprintf(
							'<span title="%1s">%2s</span>',
							__( 'This template file does not exist', 'ch-directs-plugin' ),
							$template
						);
					}
