...
namespace CH_Directs_Plugin\Fields_Import_Page;
...
<div class="wrap import-registered-field-groups">
    ...
    <?php if ( $active_tab == 'chd-registered-fields-import' ) :
        ...
        <div id="message" class="error below-h2">
            <p><?php printf( esc_html__( 'This tool was built for ACF version 5 and you have version %s.', 'ch-directs-plugin' ) ); ?></p>
        </div>
        <?php endif; ?>

        <?php if ( ! empty( $imported ) ) { ?>
        <div id="message" class="updated below-h2">
            <p><strong><?php esc_html_e( 'Field groups imported:', 'ch-directs-plugin' ); ?></strong></p>
            <ul>
        ...
        <?php }
        printf( '<p>%1s</p>', esc_html__( 'This tool imports any field groups registered outside the ACF plugin so that they can be edited.', 'ch-directs-plugin' ) ); ?>
        <?php if ( ! empty( $acf_local->groups ) ) : ?>

        <form method="POST">
            <table class="widefat">
                <thead>
                    <th><?php esc_html_e( 'Import', 'ch-directs-plugin' ); ?></th>
                    <th><?php esc_html_e( 'Registered Field Groups', 'ch-directs-plugin' ); ?></th>
                    <th><?php esc_html_e( 'Possible Existing Matches', 'ch-directs-plugin' ); ?></th>
                </thead>
                <tbody>
                    <?php
                    ...
                    <tr>
                        ...
                        if ( empty( $matches ) ) {
                            echo '<em>' . esc_html__( 'No matches found.', 'ch-directs-plugin' ) . '</em>';
                        } else {
                            $links = array();
            ...
            <p class="submit">
                <input type="submit" value="<?php esc_html_e( 'Import', 'ch-directs-plugin' ); ?>" class="button-primary" />
            </p>
        </form>

        <h3><?php esc_html_e( 'Field groups found in files:', 'ch-directs-plugin' ); ?></h3>
