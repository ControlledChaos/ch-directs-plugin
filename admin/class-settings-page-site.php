...
class Settings_Page_Site {
    ...
    public function settings_page() {
		...
		if ( chd_acf_options() ) {
			...
			// Otherwise use "Site Settings" as the label.
				...
				$label = __( 'Site Settings', 'ch-directs-plugin' );
			}
		...
		// If the Advanced Custom Fields Pro plugin is not active.
			...
			// Otherwise use the Admin Settings icon.
				...
				$icon = __( 'dashicons-admin-settings', 'ch-directs-plugin' );
			}
    ...
    public function page_help_section() {
		...
		$screen->add_help_tab( [
			'id'       => 'inline_scripts',
			'title'    => __( 'ACF Notice', 'ch-directs-plugin' ),
			'content'  => null,
    ...
    public function page_help_section_sidebar() {
		...
		$html = '<ul>
			<li><a href="https://www.advancedcustomfields.com/resources/" target="_blank" style="text-decoration: none;">' . __( 'ACF Documentation', 'ch-directs-plugin' ) . '</a></li>
		</ul>';
	...
	public function acf_docs_link() {
		...
		if ( chd_acf_options() ) {
			...
			$submenu['edit.php?post_type=acf-field-group'][] = [ __( 'Documentation', 'ch-directs-plugin' ), 'manage_options', $url ];
