...
class Frontend {
	...
	public function dependencies() {
		...
		if ( $styles ) {
			require_once CHP_PATH . 'frontend/class-styles-inline.php';

		// Otherwise enqueue styles.
		} else {
			require_once CHP_PATH . 'frontend/class-styles-enqueue.php';
		}

		// Add scripts inline if option selected.
		if ( $scripts ) {
			require_once CHP_PATH . 'frontend/class-scripts-inline.php';

		// Otherwise enqueue scripts.
		} else {
			require_once CHP_PATH . 'frontend/class-scripts-enqueue.php';
		}

		// Clean up some scripts in the `head` section.
		require_once CHP_PATH . 'frontend/class-head-scripts.php';

		// Meta tags for SEO.
		include_once CHP_PATH . 'frontend/meta-tags/class-meta-tags.php';
	...
	public function get_jquery() {
		...
		if ( ! is_customize_preview() ) {
			...
			$jquery = file_get_contents( CHP_PATH . '/assets/js/jquery.min.js' );
